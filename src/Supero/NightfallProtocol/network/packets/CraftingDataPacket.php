...
class CraftingDataPacket extends PM_Packet{
	...
	protected function encodePayload(PacketSerializer $out) : void{
		...
		foreach($this->recipesWithTypeIds as $d){
			if($d instanceof ShapedRecipe){
				$d = new CustomShapedRecipe(
					typeId: $d->getTypeId(),
					recipeId: $d->getRecipeId(),
					input: $d->getInput(),
					output: $d->getOutput(),
					uuid: $d->getUuid(),
					blockType: $d->getBlockName(),
					priority: $d->getPriority(),
					symmetric: $d->isSymmetric(),
					unlockingRequirement: $d->getUnlockingRequirement(),
					recipeNetId: $d->getRecipeNetId()
				);
			}elseif($d instanceof ShapelessRecipe){
				$d = new CustomShapelessRecipe(
					typeId: $d->getTypeId(),
					recipeId: $d->getRecipeId(),
					inputs: $d->getInputs(),
					outputs: $d->getOutputs(),
					uuid: $d->getUuid(),
