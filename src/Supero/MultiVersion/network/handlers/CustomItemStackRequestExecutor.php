...
use Supero\MultiVersion\network\packets\types\inventory\stackrequest\CustomItemStackRequestSlotInfo;
...
class CustomItemStackRequestExecutor
{
	...
	public function __construct(
		...
		private CustomItemStackRequest $request
	){
	 ...
	 * @throws ItemStackRequestProcessException
	 */
	protected function getBuilderInventoryAndSlot(CustomItemStackRequestSlotInfo  $info) : array{
	 ...
	 * @throws ItemStackRequestProcessException
	 */
	protected function transferItems(CustomItemStackRequestSlotInfo $source, CustomItemStackRequestSlotInfo $destination, int $count) : void{
	 ...
	 * @throws ItemStackRequestProcessException
	 */
	protected function removeItemFromSlot(CustomItemStackRequestSlotInfo $slotInfo, int $count) : Item{
		if($slotInfo->getContainerName()->getContainerId() === ContainerUIIds::CREATED_OUTPUT && $slotInfo->getSlotId() === UIInventorySlotOffset::CREATED_ITEM_OUTPUT){
			...
			//do we need to send a response for this slot info?
	 ...
	 * @throws ItemStackRequestProcessException
	 */
	protected function addItemToSlot(CustomItemStackRequestSlotInfo  $slotInfo, Item $item, int $count) : void{
		$this->requestSlotInfos[] = $slotInfo;
		[$inventory, $slot] = $this->getBuilderInventoryAndSlot($slotInfo);
		if($count < 1){
			//this should be impossible at the protocol level, but in case of buggy core code this will prevent exploits
	...
	protected function beginCrafting(int $recipeId, int $repetitions) : void{
		...
		if($repetitions > 256){
			//we can probably lower this limit to 64, but I'm unsure if there are cases where the client may
		...
		//Since the system assumes that crafting can only be done in the crafting grid, we have to give it a
	...
	protected function processItemStackRequestAction(ItemStackRequestAction $action) : void{
		if(
			$action instanceof CustomTakeStackRequestAction ||
			$action instanceof CustomPlaceStackRequestAction
		){
			$this->transferItems($action->getSource(), $action->getDestination(), $action->getCount());
		}elseif($action instanceof CustomSwapStackRequestAction){
		...
		}elseif($action instanceof CustomDropStackRequestAction){
			//this action has a "randomly" field, I have no idea what it's used for
			$dropped = $this->removeItemFromSlot($action->getSource(), $action->getCount());
			$this->builder->addAction(new DropItemAction($dropped));

		}elseif($action instanceof CustomDestroyStackRequestAction){
			$destroyed = $this->removeItemFromSlot($action->getSource(), $action->getCount());
			$this->builder->addAction(new DestroyItemAction($destroyed));

		}elseif($action instanceof CustomCreativeCreateStackRequestAction){
		...
		}elseif($action instanceof CustomCraftRecipeStackRequestAction){
		...
		}elseif($action instanceof CustomCraftRecipeAutoStackRequestAction){
			$this->beginCrafting($action->getRecipeId(), $action->getRepetitions());
		}elseif($action instanceof CustomCraftingConsumeInputStackRequestAction){
	...
	public function getItemStackResponseBuilder() : CustomItemStackResponseBuilder{
		$builder = new CustomItemStackResponseBuilder($this->request->getRequestId(), $this->inventoryManager);
	...
	public function buildItemStackResponse() : CustomItemStackResponse{
